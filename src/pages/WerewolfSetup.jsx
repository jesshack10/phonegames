...
export default function WerewolfSetup() {
  ...
  async function handleJoin() {
    ...
    try {
      ...
      if (!meta) { setJoinError('Session not found'); return }
      if (game !== 'werewolf') { setJoinError("That code isn't a Werewolf game"); return }
      if (Date.now() - meta.createdAt > SESSION_TTL) {
        await deleteSession(normalized)
        setJoinError('Session expired')
        return
      }
      if (meta.phase !== 'lobby') { setJoinError('This game has already started'); return }
      const avatar = generateAvatar()
      await joinSession(normalized, uid, trimmed, avatar)
      localStorage.setItem(`ww_player_${normalized}`, JSON.stringify({ playerId: uid, name: trimmed, avatar }))
    ...
    return (
      <div className="min-h-screen bg-[#0a0a18] flex flex-col items-center px-6 py-10 gap-6">
        ...
        <div className="w-full max-w-xs">
          <ShareSessionLink url={lobbyUrl} shareTitle="Werewolf" shareText={`Join the Werewolf game (code ${sessionId})`} />
        </div>
        <div className="w-full max-w-xs text-center text-white/50 text-xs">
          {roleConfig.werewolves}W · {roleConfig.seers}S · {roleConfig.doctors}D · {roleConfig.villagers}V · {playerCount} players total
        </div>
        <button onClick={() => navigate(`/werewolf/moderator/${sessionId}`)} className="w-full max-w-xs py-4 rounded-2xl bg-red-700 border border-red-500 text-white text-lg font-bold active:scale-95 transition-transform">Open Moderator Panel →</button>
        <button onClick={() => navigate('/')} className="text-white/30 text-sm">← Back to games</button>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-[#0a0a18] flex flex-col px-6 py-10 gap-6">
      ...
      <input type="text" value={joinName} onChange={e => { setJoinName(e.target.value); setJoinError('') }} placeholder="Your name…" maxLength={16} className="w-full px-4 py-3 rounded-xl bg-white/10 border border-white/20 text-white text-lg placeholder-white/30 outline-none focus:border-red-500" />
      <div className="flex gap-2">
        <input type="text" value={joinCode} onChange={e => { setJoinCode(e.target.value.toUpperCase()); setJoinError('') }} onKeyDown={e => e.key === 'Enter' && handleJoin()} placeholder="CODE" maxLength={6} autoCapitalize="characters" autoCorrect="off" autoComplete="off" spellCheck={false} className="flex-1 px-4 py-3 rounded-xl bg-white/10 border border-white/20 text-white text-center text-lg font-mono font-bold tracking-widest placeholder-white/30 outline-none focus:border-white/40" />
        <button onClick={handleJoin} disabled={!ready || !uid || !joinName.trim() || joinCode.trim().length !== 6 || loading || joining} className="px-5 py-3 rounded-xl bg-white/10 active:bg-white/20 text-white font-bold disabled:opacity-30 transition-colors">{joining ? '…' : 'Join →'}</button>
      </div>
      ...
      <div>
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => handlePlayerCountChange(Math.max(4, playerCount - 1))} className="w-10 h-10 rounded-full bg-white/10 text-white text-xl font-bold active:bg-white/20">−</button>
          <span className="text-white text-3xl font-bold w-10 text-center">{playerCount}</span>
          <button onClick={() => handlePlayerCountChange(Math.min(20, playerCount + 1))} className="w-10 h-10 rounded-full bg-white/10 text-white text-xl font-bold active:bg-white/20">+</button>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-3">
          {[
            ...
            <div key={key} className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <button onClick={() => setRole(key, -1)} disabled={roleConfig[key] <= min} className="w-8 h-8 rounded-full bg-white/10 text-white font-bold disabled:opacity-30 active:bg-white/20">−</button>
                <span className="text-white font-bold w-5 text-center">{roleConfig[key]}</span>
                <button onClick={() => setRole(key, 1)} className="w-8 h-8 rounded-full bg-white/10 text-white font-bold active:bg-white/20">+</button>
              </div>
            </div>
          ))}
      ...
      <button onClick={handleCreate} disabled={!isValid || loading || joining || !ready || !uid} className="w-full py-4 rounded-2xl bg-red-700 border border-red-500 text-white text-lg font-bold disabled:opacity-40 active:scale-95 transition-transform mt-auto">
