...
export default function PeticionesLobby() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        ...
        if (uid === storedUid) navigate(`/peticiones/play/${sessionId}`, { replace: true })
  ...
  async function handleJoin() {
    ...
    try {
      ...
      if (!m) { setError('Sesión no encontrada'); setLoading(false); return }
      if (Date.now() - m.createdAt > SESSION_TTL) { await deleteSession(sessionId); setError('Sesión expirada'); setLoading(false); return }
    ...
    } catch { setError('Error al unirte. Intenta de nuevo.'); setLoading(false) }
  }

  return (
    <div className="min-h-screen bg-[#0a0a18] flex flex-col items-center justify-center px-6 gap-6">
      <div className="text-7xl">🙏</div>
      <h1 className="text-white text-3xl font-black tracking-tight">PETI<span className="text-blue-400">CIONES</span></h1>
      <p className="text-white/30 text-sm font-mono tracking-widest">{sessionId}</p>
      <div className="w-full max-w-xs flex flex-col gap-3">
        <input type="text" placeholder="Escribe tu nombre…" value={name} onChange={e => { setName(e.target.value); setError('') }} onKeyDown={e => e.key === 'Enter' && handleJoin()} maxLength={24} autoFocus className="w-full px-4 py-4 rounded-xl bg-white/10 border border-white/20 text-white text-lg placeholder-white/30 outline-none focus:border-blue-500" />
        {error && <p className="text-red-400 text-sm">{error}</p>}
        <button onClick={handleJoin} disabled={!name.trim() || !ready || loading} className="w-full py-4 rounded-2xl bg-blue-500 active:bg-blue-600 text-white text-lg font-bold disabled:opacity-40 transition-colors">{loading ? 'Uniendo…' : 'Unirme →'}</button>
      </div>
    </div>
  )
}
