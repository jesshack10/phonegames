...
export default function WerewolfPlayer() {
  ...
  useEffect(() => {
    ...
    if (!stored) navigate(`/werewolf/lobby/${sessionId}`)
  }, [sessionId, navigate, storageKey])

  useEffect(() => {
    const unsubs = [subscribeSession(sessionId, setMeta), subscribePlayers(sessionId, setPlayers)]
  ...
  useEffect(() => {
    if (meta) {
      ...
      if (Date.now() - meta.createdAt > SESSION_TTL) deleteSession(sessionId).then(() => navigate('/', { replace: true }))
  ...
  if (!meta || !myPlayer) return <div className="min-h-screen bg-[#0a0a18] flex items-center justify-center"><p className="text-white/50">Loading…</p></div>
  ...
  if (phase === 'ended') {
    ...
    return (
      <div className="min-h-screen bg-[#0a0a18] flex flex-col items-center justify-center px-6 gap-6">
        ...
        <button onClick={() => navigate('/')} className="text-white/40 text-sm mt-4">← Back to games</button>
      </div>
    )
  }
  ...
  if (!myAlive) {
    return (
      <div className="min-h-screen bg-[#0a0a18] flex flex-col items-center px-6 py-10 gap-5">
        ...
        <div className={`rounded-xl border-2 ${roleConfig.bg} ${roleConfig.border} px-6 py-3 text-center`}><p className={`font-bold ${roleConfig.text}`}>{roleConfig.emoji} You were the {roleConfig.label}</p></div>
        <p className="text-white/40 text-sm">Spectating…</p>
        <div className="w-full max-w-xs"><p className="text-white/40 text-xs uppercase tracking-widest mb-2">Living players</p><PlayerList players={livingPlayers} compact /></div>
        <p className="text-white/30 text-xs mt-2 text-center">Phase: {phase?.replace('_', ' ')} · Round {meta.round}</p>
      </div>
    )
  }
  ...
  if (phase === 'night') {
    if (!isMyNightTurn || myActionSubmitted) {
      return (
        <div className="min-h-screen bg-[#0a0a18] flex flex-col items-center justify-center px-6 gap-6">
          ...
          {myActionSubmitted ? (
            <><p className="text-white text-xl font-semibold">Action submitted ✓</p><p className="text-white/40 text-sm">Waiting for the night to end…</p></>
          ) : (
            <><p className="text-white text-xl font-semibold">Close your eyes</p><p className="text-white/40 text-sm">{nightTurn === 'werewolves' ? 'Werewolves are acting…' : nightTurn === 'doctor' ? 'Doctor is acting…' : 'Seer is acting…'}</p></>
          )}
          <Avatar avatar={myPlayer.avatar} size="lg" />
    ...
    const nightTargets = livingPlayers.filter(p => myRole === 'werewolf' ? p.role !== 'werewolf' : true)
    const actionLabel = myRole === 'werewolf' ? '🐺 Choose your victim' : myRole === 'doctor' ? '💉 Choose who to protect' : '🔮 Choose who to investigate'
    return (
      <div className="min-h-screen bg-[#0a0a18] flex flex-col px-6 py-10 gap-6">
        ...
        {myRole === 'seer' && nightActions.seerTarget && nightActions.seerResult !== undefined && nightActions.seerResult !== null && (
          <div className="rounded-xl bg-indigo-900/40 border border-indigo-600 p-4 text-center">
            <p className="text-indigo-300 font-semibold">{livingPlayers.find(p => p.id === nightActions.seerTarget)?.name} is {nightActions.seerResult ? '🐺 a Werewolf!' : '✓ not a werewolf'}</p>
          </div>
        )}
        <NightActionPanel players={nightTargets} onConfirm={handleNightAction} confirmLabel={myRole === 'werewolf' ? 'Confirm kill →' : myRole === 'doctor' ? 'Confirm protection →' : 'Investigate →'} disabled={myActionSubmitted} excludeSelf={myRole === 'seer'} myId={uid} />
      </div>
    )
  }

  if (phase === 'day_announce') return <div className="min-h-screen bg-[#0a0a18] flex flex-col items-center justify-center px-6 gap-6"><div className="text-6xl">🌅</div><p className="text-white text-xl font-semibold text-center">Morning arrives…</p><p className="text-white/40 text-sm text-center">The moderator is revealing last night's events.</p></div>
  if (phase === 'day_discuss') return <div className="min-h-screen bg-[#0a0a18] flex flex-col items-center px-6 py-10 gap-5"><div className="text-5xl">☀️</div><h2 className="text-white text-2xl font-bold text-center">Day Discussion</h2><p className="text-white/50 text-sm text-center">Discuss with the group. Find the werewolves.</p><div className="w-full max-w-xs mt-2"><PlayerList players={livingPlayers} compact /></div><p className="text-white/30 text-xs mt-auto">Waiting for moderator to open voting…</p></div>
  if (phase === 'voting') return <div className="min-h-screen bg-[#0a0a18] flex flex-col px-6 py-10 gap-6"><h2 className="text-white text-2xl font-bold text-center">🗺 Vote to Eliminate</h2><VotePanel players={livingPlayers} onVote={handleVote} myId={uid} /></div>
  if (phase === 'day_elimination') return <div className="min-h-screen bg-[#0a0a18] flex flex-col items-center justify-center px-6 gap-6"><div className="text-5xl">💣</div><p className="text-white text-xl font-semibold text-center">The village has spoken</p><p className="text-white/40 text-sm text-center">Waiting for the next night to begin…</p></div>

  return <div className="min-h-screen bg-[#0a0a18] flex items-center justify-center"><p className="text-white/40">Syncing…</p></div>
}
