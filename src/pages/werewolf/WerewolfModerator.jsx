...
export default function WerewolfModerator() {
  ...
  const werewolvesInGame = gamePlayers.filter(p => p.role === 'werewolf' && p.alive)
  const doctorInGame = gamePlayers.find(p => p.role === 'doctor' && p.alive)
  const seerInGame = gamePlayers.find(p => p.role === 'seer' && p.alive)
  ...
  async function handleAdvanceTurn() {
    ...
    if (next) {
      ...
      // All night turns done — resolve night
      ...
      await appendActionLog(sessionId, meta.round, {
        ...
        payload: {
          description: result.killed
            ? `Werewolves killed ${getPlayerName(result.killed)}${result.savedByDoctor ? ' (saved by doctor)' : ''}`
      ...
      if (nightActions.doctorTarget) {
        await appendActionLog(sessionId, meta.round, {
          type: 'doctor_save',
          payload: { description: `Doctor protected ${getPlayerName(nightActions.doctorTarget)}` },
        })
      }
      if (nightActions.seerTarget) {
        const target = gamePlayers.find(p => p.id === nightActions.seerTarget)
        await appendActionLog(sessionId, meta.round, {
          type: 'seer_check',
          payload: { description: `Seer checked ${getPlayerName(nightActions.seerTarget)}: ${target?.role === 'werewolf' ? 'WEREWOLF' : 'not a werewolf'}` },
        })
      }
      ...
      const updatedPlayers = gamePlayers.map(p =>
        p.id === result.killed ? { ...p, alive: false } : p
      )
      const winner = checkWinCondition(updatedPlayers)
      if (winner) {
        await endGame(sessionId, winner)
      }
    }
  }
  ...
  async function handleConfirmElimination() {
    ...
    if (eliminatedId) {
      await appendActionLog(sessionId, meta.round, {
        type: 'elimination',
        payload: { description: `${getPlayerName(eliminatedId)} was eliminated by vote` },
      })
    } else {
      await appendActionLog(sessionId, meta.round, {
        type: 'no_elimination',
        payload: { description: 'No elimination this round' },
      })
    }
    ...
    return (
      <div className="min-h-screen bg-[#0a0a18] flex flex-col items-center px-6 py-10 gap-6">
        ...
        <div className="w-full max-w-xs">
          <p className="text-white/50 text-xs uppercase tracking-widest mb-3">All roles</p>
    ...
    <div className="min-h-screen bg-[#0a0a18] flex flex-col px-4 py-6 gap-5 pb-10">
      ...
      {meta.phase === 'role_reveal' && (
        <>
          ...
          <button
            ...
            className="w-full py-4 rounded-2xl bg-indigo-700 border border-indigo-500 text-white text-lg font-bold active:scale-95 transition-transform"
          >
            🌙 Begin Night →
          </button>
        </>
        ...
        <>
          ...
          <div className="flex flex-col gap-2 text-sm">
            {nightActions.werewolfTarget && (
              <div className="rounded-lg bg-red-900/30 border border-red-800 px-3 py-2 text-red-300">
                🐺 Werewolves chose: <strong>{getPlayerName(nightActions.werewolfTarget)}</strong>
              </div>
            )}
            {nightActions.doctorTarget && (
              <div className="rounded-lg bg-teal-900/30 border border-teal-800 px-3 py-2 text-teal-300">
                💉 Doctor protected: <strong>{getPlayerName(nightActions.doctorTarget)}</strong>
              </div>
            )}
            {nightActions.seerTarget && (
              <div className="rounded-lg bg-indigo-900/30 border border-indigo-800 px-3 py-2 text-indigo-300">
                🔮 Seer checked: <strong>{getPlayerName(nightActions.seerTarget)}</strong>
                {nightActions.seerResult !== null && nightActions.seerResult !== undefined && (
                  <span className="ml-2">→ {nightActions.seerResult ? '🐺 Werewolf' : '✓ Not a werewolf'}</span>
                )}
              </div>
            )}
          </div>
      ...
      {meta.phase === 'day_announce' && (
        <>
            ...
            return (
              <div className={`rounded-xl border p-5 text-center ${actualKilled ? 'bg-red-900/30 border-red-700' : 'bg-green-900/30 border-green-700'}`}>
                ...
                <p className="text-white text-xl font-bold">
                  {actualKilled ? `${getPlayerName(actualKilled)} was killed` : 'Nobody died tonight'}
                </p>
          ...
          <PlayerList players={gamePlayers} showRoles compact />
          <button
            onClick={() => startDiscussion(sessionId)}
            className="w-full py-4 rounded-2xl bg-amber-700 border border-amber-500 text-white text-lg font-bold active:scale-95 transition-transform"
          >
            ☀️ Start Discussion →
          </button>
        </>
        ...
        <>
          ...
          <div className="flex flex-col gap-2">
            {tally.map(player => (
              <button
                key={player.id}
                onClick={() => setSelectedElimination(prev => prev === player.id ? null : player.id)}
                className={`flex items-center gap-3 rounded-xl px-3 py-2 border transition-colors ${
                  selectedElimination === player.id
                    ? 'bg-red-900/50 border-red-500'
                    : tied.find(t => t.id === player.id) && maxVotes > 0
                ...
                <div className="flex items-center gap-2">
                  <div className="w-24 h-2 bg-white/10 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: livingPlayers.length > 0 ? `${(player.votes / livingPlayers.length) * 100}%` : '0%' }}
                    />
                  </div>
          ...
          <button
            onClick={() => setSelectedElimination(null)}
            className={`text-sm text-white/40 text-center ${selectedElimination === null ? 'underline text-white/60' : ''}`}
          >
            No elimination
          </button>
          ...
          <button
            ...
            className="w-full py-4 rounded-2xl bg-red-700 border border-red-500 text-white text-lg font-bold active:scale-95 transition-transform"
          >
